...
def disasm(opcode):
    ...
    elif ins == 0x1: dissasmStr = f'JP   0x{nnn:x}'
    ...
    elif ins == 0x6: dissasmStr = f'LD   V{x:x} 0x{kk:x}'
    elif ins == 0x7: dissasmStr = f'ADD  V{x:x} 0x{kk:x}'
    elif ins == 0x9: dissasmStr = f'SNE  V{x:x} V{y}'
    elif ins == 0xa: dissasmStr = f'LD   [I] 0x{nnn:x}'
    elif ins == 0xc: dissasmStr = f'RND  V{x:x}  0x{kk:x}'
    elif ins == 0xd: dissasmStr = f'DRAW V{x:x}  V{y:x} 0x{n:x}'
    
    # ==[ input operation ]==
    elif ins == 0xe and (opcode & 0xff) == 0x9e: dissasmStr = f'skp_pressed'
    elif ins == 0xe and (opcode & 0xff) == 0xa1: dissasmStr = f'skp_not_pressed'
    
    # ==[ logical operation ]==
    elif ins == 0x8 and (opcode & 0xf) == 0x0: dissasmStr = f'LD   V{x:x} V{y:x}'
    ...
    elif ins == 0xf and (opcode & 0xff) == 0x07: dissasmStr = f'fx07(x)'
    elif ins == 0xf and (opcode & 0xff) == 0x0a: dissasmStr = f'LD   V{x:x},  K'
    elif ins == 0xf and (opcode & 0xff) == 0x55: dissasmStr = f'LD   [I], V{x:x}'
    elif ins == 0xf and (opcode & 0xff) == 0x1e: dissasmStr = f'ADD  [I], V{x:x}'
    elif ins == 0xf and (opcode & 0xff) == 0x15: dissasmStr = f'fx15(x)'
    elif ins == 0xf and (opcode & 0xff) == 0x18: dissasmStr = f'fx18(x)'
